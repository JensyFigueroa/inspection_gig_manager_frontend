...
import AvgDPUChart from "./AvgDPUChart";
import PrintModal from "../PrintModal/PrintModal.jsx";
...
function DPUTracker() {
  ...
  const fetchAvgDPUData = async () => {
    try {
      const response = await authAxios.get(
        `gigs/dpu-history?year=${new Date().getFullYear()}`,
      );
  ...
  // Llamar cuando se abre el modal de impresión
  ...
  const getGrandTotal = () =>
    trucks.reduce((sum, truck) => sum + getColumnTotal(truck.truckNumber), 0);
  ...
  const exportToExcel = async () => {
    ...
    STATIONS.forEach((station, idx) => {
      const rowData = [
        ...
        idx === 0 ? "GIGS" : "",
        ...trucks.map((t) =>
          t.truckNumber ? gigsByStation[station]?.[t.truckNumber] || 0 : "",
        ),
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        ...
        <div className="header-right">
          ...
          <button className="btn-print" onClick={handleOpenPrintModal}>
            🖨️ Print
          </button>
        </div>
      </header>
